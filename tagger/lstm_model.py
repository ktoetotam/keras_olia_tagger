...
y_enc = [[0] * (maxlen - len(ey)) + [label2ind[c] for c in ey] for ey in Y] #ey - sequence output in Y, c is a label in the sequence
y_enc = [[encode(c, max_label) for c in ey] for ey in y_enc]

X_enc = pad_sequences(X_enc, maxlen=maxlen)
y_enc = pad_sequences(y_enc, maxlen=maxlen)

X_train, X_test, y_train, y_test = train_test_split(X_enc, y_enc, test_size=4*32, train_size=15*32, random_state=42)
